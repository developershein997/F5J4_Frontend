...
function useUser() {
  ...
  useEffect(() => {
    fetch('https://www.delightmyanmar99.pro/api/user', {
      headers: {
...
export default function DigitBetGame() {
  ...
  useEffect(() => {
    fetch('https://www.delightmyanmar99.pro/api/digitbet/history', {
      headers: {
        'Authorization': 'Bearer ' + localStorage.getItem('token'),
        'Accept': 'application/json'
      }
  ...
  const sendToBackend = async (bet, resultNumber, winStatus, digit = null) => {
    ...
    try {
      ...
      const response = await fetch('https://www.delightmyanmar99.pro/api/digitbet', {
        method: 'POST',
        headers: {
