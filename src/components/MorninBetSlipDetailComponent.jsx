...
const MorninBetSlipDetailComponent = ({ slip, onBack }) => {
  if (!slip) return null;
  ...
  return (
    <div className="p-6 bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl mx-auto text-white font-inter border border-gray-700">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-bold text-teal-400">လောင်းကြေးစာရင်း အသေးစိတ်</h2>
        <button
          onClick={onBack}
        ...
        {slip.two_bets?.map((betItem) => (
          <div key={betItem.id} className="bg-gray-700 p-4 rounded-lg shadow-md flex justify-between items-center">
            <div>
              ...
              <p className="text-md text-gray-400">ပမာဏ: ${parseFloat(betItem.bet_amount).toFixed(2)}</p>
            </div>
            <div className="text-right">
              ...
              <p className="text-md text-gray-400">ဖြစ်နိုင်ခြေရှိသော ပေးချေမှု: ${parseFloat(betItem.potential_payout).toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
...
export default MorninBetSlipDetailComponent;
